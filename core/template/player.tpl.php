...
<div id="video-box-<?= $uid ?>" class="video-box" style="width:<?= $width ?>px; min-height:<?= ($height + 28) ?>px;">
	...
	<div id="mycontrols-<?= $uid ?>" style="display:none;">
		...
		<div id="progressbox-<?= $uid ?>" class="progressbox" style="width:<?= ($width - 2) ?>px;">
...
<script type="text/javascript">
	...
	document.getElementById("video-box-<?= $uid ?>").setAttribute("style",
		"width:"+(<?=$width?>/scale)+"px;min-height:"+(<?=($height+28)?>/scale)+"px");
	document.getElementById("player-<?= $uid ?>").setAttribute("style",
		"width:"+(<?= $width ?>/scale)+"px;height:"+(<?= $height ?>/scale)+"px");
	document.getElementById("progressbox-<?= $uid ?>").setAttribute("style",
		"width:"+(<?= ($width - 2) ?>/scale)+"px");
	...
	if(!supportsVideo || !supportsH264()) {
		...
		// $('#player-<?= $uid ?>').click(player<?= $uid ?>.play());
	...
	function startCount() {
		t = window.setInterval(function() {
			...
			if (player<?= $uid ?>.duration) {
				$('#progress-<?= $uid ?>').width(player<?= $uid ?>.currentTime / player<?= $uid ?>.duration * <?= ($width - 2) ?>/scale);
				$('#buffered-<?= $uid ?>').width(player<?= $uid ?>.buffered.end(0) / player<?= $uid ?>.duration * <?= ($width - 2) ?>/scale);
